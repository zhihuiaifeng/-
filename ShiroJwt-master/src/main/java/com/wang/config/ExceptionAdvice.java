...
public class ExceptionAdvice {
    ...
    @ResponseBody
    public ResponseBean error(HttpServletRequest request, HttpServletResponse response, Exception e) {
        e.printStackTrace();
        if(e.getClass() == TokenExpiredException.class) {
            //类型转型
            TokenExpiredException ce = (TokenExpiredException) e;
            ResponseBean result = new ResponseBean(800,ce.getMessage(),null);
            return result;
        }else{
            ResponseBean result = new ResponseBean(801,"刷新token出现问题",null);
            return result;
        }
    }
}
